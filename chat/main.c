...
void renderHistory(char* message, const char *username)
{
    if (addMessageToHistory(message, username))
    {
        ...
        for (unsigned char messageIndex = 0; messageIndex < HISTORY_LENGTH; ++messageIndex)
        {
            for (unsigned char charIndex = 0; charIndex < LINE_LENGTH_WITH_USERNAME; ++charIndex)
            {
                unsigned char printableChar = allMessages[messageIndex][charIndex];
                if (printableChar == 0)
                {
                    break;
                }
...
void setup()
{
    ...
    {
        keyboardRead();
        gpuDrawLine_c(10,100,10+k_char,100,COLOR_BLACK);
